#fastq_to_bam_interleaved.snakefile
#Anna Hoge
...
snakemake -s fastq_to_bam_interleaved.snakefile --latency-wait 60 --restart-times 3 --keep-going --cluster-config config/cluster_slurm.yaml --cluster "sbatch -p {cluster.partition} --mem={cluster.mem} -t {cluster.time} -c {cluster.ncpus} -n {cluster.ntasks} -o {cluster.output}" -j 100 -np
"""
...
rule map_to_reference:
    input:
        fastq=lambda wildcards: config["samples"][wildcards.samples]
    output:
    ...
    shell:
        "({params.bwa} mem -t {params.bwa_threads} -p -M \
        -R '@RG\\tID:no_id\\tLB:no_library\\tPL:no_platform\\tPU:no_unit\\tSM:{wildcards.samples}' \
        {params.reference_genome} \
        {input.fastq} | {params.samtools} view -b - > {output}) 2> {log}"
