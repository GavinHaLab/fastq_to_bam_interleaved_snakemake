...
snakemake -s fastq_to_bam_interleaved.snakefile --latency-wait 60 --keep-going --cluster-config config/cluster_slurm.yaml --cluster "sbatch -p {cluster.partition} --mem={cluster.mem} -t {cluster.time} -c {cluster.ncpus} -n {cluster.ntasks} -o {cluster.output}" -j 100 -np
"""
...
rule map_to_reference:
    ...
    shell:
        ...
        {input.fastq} | {params.samtools} view -b - > {output}"
